...
export default function SharePost({ title, slug, description }: props) {
  const blog = encodeURIComponent('https://brianriant.vercel.app/blog');
  const options = [
    {
      icon: BiLogoTwitter,
      name: 'Twitter',
      shareUrl: `https://twitter.com/intent/tweet?text=${encodeURIComponent(
        'Thank you @brianriant for writing this post.'
    ...
    },
    {
      ...
      shareUrl: `https://api.whatsapp.com/send?text=${encodeURIComponent(
        'Read this amazing article by Brian Riant'
      )}.%0A%0A${title}%0A%0A${blog}${slug}`,
    },
  ];
      ...
      <div className="flex flex-wrap items-center gap-2 tracking-tight">
        {options.map((data, id) => (
          <button
            ...
            className="w-12 h-12 p-2 grid place-content-center text-2xl  border dark:border-green-800 border-green-200 rounded-md"
          >
